...
class DecryptionStream implements StreamInterface
{
    ...
    public function read($length): string
    {
        ...
        $this->buffer .= $data;
        if (strlen($this->buffer) < $length + 10) {
            return '';
        }

        $encrypted = substr($this->buffer, 0, -10);
        $mac = substr($this->buffer, -10);

        $computedMac = StreamHelper::generateMac($this->iv . $encrypted, $this->macKey);
        if ($mac !== $computedMac) {
            throw new RuntimeException('MAC validation failed');
        }

        $decrypted = StreamHelper::decrypt($encrypted, $this->cipherKey, $this->iv);
